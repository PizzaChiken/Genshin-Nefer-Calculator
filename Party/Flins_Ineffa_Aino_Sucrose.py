...
from Characters.Sucrose import SucroseClass
...
from Weapons.Catalyst.WanderingEvenstar import WanderingEvenstar
...
from Artifacts.ViridescentVenerer import ViridescentVenerer
...
# 6. 캐릭터 4 추가
SucroseConstellation = 6
SucroseRefinements = 5
Sucrose = SucroseClass(Calculator, Constellation=SucroseConstellation, UltElement='Electro', P1Element=['Electro'])
Sucrose.AddWeapon(WanderingEvenstar(Calculator, Sucrose, SucroseRefinements))
Sucrose.AddArtifactSet(ViridescentVenerer(Calculator, Sucrose, PC=4, Elements=['Electro']))
Sucrose.AddArtifacts([
    {'AdditiveHP':4780, 'AdditiveATK':311, '%HP':0.466*0, '%ATK':0.466*0, '%DEF':0.583*0, 'EM':187*3, 'ER':0.518*0, 'DendroDMGBonus':0.466*0, 'PhysicalDMGBonus':0.583*0, 'CR':0.311*0, 'CD':0.622*0}, # 주옵
    {'AdditiveHP':254*0, 'AdditiveATK':17*0, 'AdditiveDEF':20*0, '%HP':0.050*0, '%ATK':0.050*5, '%DEF':0.062*0, 'EM':20*6, 'ER':0.055*15, 'CR':0.033*2, 'CD':0.066*2},
    ]) 
Calculator.AddCharacter(Sucrose)

# 7. 파티효과 적용
Calculator.ApplyPartyEffect(NotNordCharacter=Sucrose, ElementalResonance=['Electro'])
...
TotalDMG += Sucrose.Rotation(Enemy)
TotalDMG += Calculator.LunarChargedDamageRotation([Flins, Ineffa, Aino, Sucrose], Enemy, Count=8)
...
print(f'{'설탕':8} {SucroseConstellation}돌 {SucroseRefinements}재, 사이클데미지: {Sucrose.TotalDamageDealt:.0f}')
